...
class FragmentMatch:
    ...
    def __repr__(self) -> str:
        return (
            f"FragmentMatch(fragment={repr(self.fragment)}, "
            f"obs_mz={self.obs_mz:.4f}, intensity={self.intensity:.2e})"
        )

    def asdict(self) -> dict[str, Any]:
        return {
            "theo_mz": self.fragment.mz,
            "obs_mz": self.obs_mz,
            "ion_type": self.fragment.ion_type,
            "charge": self.fragment.charge_state,
            "pos": self.fragment.position,
            "error": self.error,
