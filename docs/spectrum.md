...
Encode the spectrum as a [spectrl](https://github.com/tacular-omics/spectrl) `spectrl.…` URL-safe token, or decode one back to a `Spectrum`/`MsnSpectrum`. The token mirrors mzML semantics (PSI-MS CV params in a single compressed CBOR document with an integrity checksum) and is suitable for sharing in URLs, QR codes, notebooks, and papers.
