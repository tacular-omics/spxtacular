...
- `MzSpecLibReader` streams an mzSpecLib library one `LibraryEntry` at a time in constant memory (text and JSON, gzipped or not), with the library attributes available before iterating. `read_mzspeclib` is built on it and returns the same results.
