...
**spxtacular** organizes this workflow around a `Spectrum` object whose processing methods return another `Spectrum`, allowing operations to be called in sequence. The object keeps charge assignments, ion-mobility values, isotope scores, and other peak-level metadata aligned with their peaks. Researchers can inspect each stage without managing detached arrays or intermediate files. The same objects feed isobaric reporter-ion quantification, reading and writing of standard spectral libraries, and figures drawn for print or for interactive use.

![Overview of the spxtacular workflow. A chainable `Spectrum` pipeline transforms raw centroids through denoising, deconvolution, neutral-mass conversion, fragment matching, and scoring. The code strip shows the corresponding calls: neutral b and y fragments from `peptacular` are scored against the decharged spectrum. File readers (including mzSpecLib), Numba-accelerated deconvolution, publication figures, isobaric reporter-ion quantification, and interoperability with other tools support the workflow.](figures/graphical_abstract.png){ #fig:abstract }
...
spxtacular builds on NumPy [@harris2020numpy], pandas [@mckinney2010pandas], and Plotly [@plotly2015]. Physical constants, element data, and isobaric-tag definitions come from `tacular` [@tacular], the reference-data package shared with `peptacular` and `paftacular`, so the three libraries agree on masses such as the proton mass.

## Figures

Plots are generated directly from the spectrum and its peak-level metadata. They include stick, mirror, fragment-annotated, mass-error, sequence-coverage, faceted, extracted-ion chromatogram, and reporter-ion plots, along with a peak table for screen readers. A backend-neutral figure layer draws each plot with Plotly for interactive exploration or with Matplotlib [@hunter2007matplotlib] for print. Named styles set fonts, line widths, and resolution for papers, screens, and slides. The paper style uses 7 pt text, standard journal column widths (85, 114, or 175 mm, or a custom size), and 600 dpi output, and Matplotlib figures are written as PDF, SVG, or EPS with embedded fonts. Fragment labels are typeset with subscript positions and superscript charges. `compose_figure` arranges several plots as one lettered multi-panel figure, so a figure for a manuscript can be produced from the same objects used during analysis.
...
Isobaric labelling with tandem mass tags (TMT and TMTpro) [@thompson2003tmt] or iTRAQ [@ross2004itraq] is quantified from reporter ions. `extract_reporter_ions` takes the most intense peak within a tolerance (20 ppm by default) of each channel's reporter m/z, and `reporter_ion_table` returns one row per spectrum for many spectra or an open reader. Channel definitions come from `tacular`. Optional isotope-impurity correction builds a correction matrix from the reagent lot sheet and solves it by non-negative least squares, so corrected intensities cannot become negative.

## Readers and interoperability

`Reader` examines the suffix of an input path and selects the matching format-specific parser. It supports Bruker timsTOF `.d` data directories, mzML files [@martens2011mzml], Thermo `.raw` files, and MGF, MS2, or MSP peak-list files. Every parser returns the same `MsnSpectrum` type with available acquisition metadata, such as native identifier, scan number, MS level, retention time, and precursor information. Every reader can also fetch a single spectrum by scan number, native identifier, or the `scannr` value reported by the Sage search engine [@lazear2023sage], which joins search results to their spectra. The peak-list parsers have no extra dependencies. The mzML and Bruker parsers use mzmlpy [@mzmlpy] and tdfpy [@tdfpy], respectively, while Thermo RAW support requires a .NET runtime.

Spectrum interpretation follows community standards from the HUPO Proteomics Standards Initiative. Fragment matching uses fragments that the companion `peptacular` package generates from ProForma peptidoform notation [@leduc2022proforma], including the [2.1 specification](https://www.psidev.info/proforma). The companion `paftacular` package [@paftacular] labels matched peaks in the mzPAF peak annotation format [@klein2024mzpaf]. Spectral libraries are read and written in the companion mzSpecLib 1.0 format [@klein2024mzpaf], as text or JSON. Each library entry keeps its spectrum, ProForma analytes, scores, and mzPAF peak annotations, and a streaming reader processes large libraries one entry at a time. The MSP and MGF writers can also add mzPAF annotations to each peak. Together with mzML input, these standards keep spxtacular inputs and annotations portable across other tools that support them. Matches can be evaluated with a base-10 hyperscore following the dot-product and factorial structure of X!Tandem [@craig2004tandem], with unit theoretical intensities and deduplicated peak contributions, the normalized spectral contrast angle [@toprak2014spectralangle], matched fraction, and mass-error statistics. Spectra can be compared with cosine, modified-cosine [@watrous2012networking], and spectral-entropy [@li2021entropy] similarity, and public spectra can be fetched by Universal Spectrum Identifier [@deutsch2021usi] from PROXI services. The optional `spectrl` package [@spectrl] encodes spectra as URL-safe tokens for sharing a specific spectrum without attaching a file.
