...
* `Spectrum.decharge(inplace=False)` called on an already-decharged spectrum now returns a copy instead of aliasing `self`; previously the caller's original spectrum could be silently mutated through the "new" object.
* `spxtacular.score()` (the standalone scoring entry point) default `tolerance_type` changed from `PPM` to `DA`, matching `Spectrum.score()` — the two had drifted apart, so calling either with default arguments on the same inputs silently produced very different match counts.
