...
def test_closest_match_da_within_tolerance() -> None:
    ...
    result = match_fragments(spec, [frag], mz_tol=0.02, mz_tol_type="Da", peak_selection="closest")
    assert len(result) == 1
    assert result[0][0] == 0  # peak at index 0 (100.0)
...
def test_exact_mz_match_da() -> None:
    ...
    result = match_fragments(spec, [frag], mz_tol=0.02, mz_tol_type="Da", peak_selection="closest")
    assert len(result) == 1
    assert result[0][0] == 2  # index of 300.0
...
def test_ppm_tolerance_match() -> None:
    ...
    result = match_fragments(spec, [frag], mz_tol=10, mz_tol_type="ppm", peak_selection="closest")
    assert len(result) == 1
    assert result[0][0] == 1  # peak at 200.0
...
def test_closest_picks_nearest_peak() -> None:
    ...
    result = match_fragments(spec, [frag], mz_tol=0.02, mz_tol_type="Da", peak_selection="closest")
    assert len(result) == 1
    assert result[0][0] == 1  # 200.015 is closer to 200.01
...
def test_largest_picks_highest_intensity_peak() -> None:
    ...
    frag = _make_frag(200.01)
    result = match_fragments(spec, [frag], mz_tol=0.02, mz_tol_type="Da", peak_selection="largest")
    assert len(result) == 1
    assert result[0][0] == 0  # 200.0 has higher intensity
...
def test_all_returns_both_peaks_in_tolerance() -> None:
    ...
    frag = _make_frag(200.01)
    result = match_fragments(spec, [frag], mz_tol=0.02, mz_tol_type="Da", peak_selection="all")
    assert len(result) == 2
    assert {r[0] for r in result} == {0, 1}
...
def test_no_fragments_returns_empty() -> None:
    spec = _spectrum()
    result = match_fragments(spec, [], mz_tol=0.02, mz_tol_type="Da")
    assert result == []
...
def test_results_sorted_by_peak_index_ascending() -> None:
    ...
    result = match_fragments(spec, frags, mz_tol=0.02, mz_tol_type="Da", peak_selection="closest")
...
def test_multiple_fragments_match_same_peak_all_appear() -> None:
    ...
    result = match_fragments(spec, [f1, f2], mz_tol=0.02, mz_tol_type="Da", peak_selection="closest")
    assert len(result) == 2
    assert all(r[0] == 0 for r in result)


def test_fragment_fragment_tuple_structure() -> None:
    spec = _spectrum()
    frag = _make_frag(200.0)
    result = match_fragments(spec, [frag], mz_tol=0.02, mz_tol_type="Da")
    assert len(result) == 1
    peak_idx, matched_frag = result[0]
    assert isinstance(peak_idx, int)
    assert matched_frag is frag
...
def test_charge_filter_all_mode() -> None:
    ...
    result = match_fragments(spec, [frag], mz_tol=0.02, mz_tol_type="Da", peak_selection="all")
    assert len(result) == 1
    assert result[0][0] == 0  # only the z=1 peak
