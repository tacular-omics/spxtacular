...
def test_deconvolve_works_without_numba() -> None:
    ...
    with numba_blocked() as deconvolve_spectrum:
        result = deconvolve_spectrum(mz, intensity, charge_range=(1, 3), tolerance=50.0, tolerance_unit="ppm")
...
def test_numba_and_pure_python_produce_identical_results() -> None:
    ...
    with numba_blocked() as pure_deconvolve:
        pure = pure_deconvolve(mz, intensity, charge_range=(1, 4), tolerance=20.0, tolerance_unit="ppm")

    from spxtacular.decon.scored import deconvolve_spectrum as current

    jitted = current(mz, intensity, charge_range=(1, 4), tolerance=20.0, tolerance_unit="ppm")
...
def test_result_arrays_have_same_length() -> None:
    ...
    mz_out, charges_out, intensity_out, scores_out = deconvolve_spectrum(
        mz, intensity, charge_range=(1, 3), tolerance=50.0, tolerance_unit="ppm"
    )
...
def test_result_mz_sorted_ascending() -> None:
    ...
    mz_out, _, _, _ = deconvolve_spectrum(mz, intensity, charge_range=(1, 3), tolerance=50.0, tolerance_unit="ppm")
...
def test_scores_in_zero_to_one_range() -> None:
    ...
    mz = np.array([500.0, 500.501, 501.002], dtype=np.float64)
    intensity = np.array([100000.0, 70000.0, 30000.0], dtype=np.float64)
    _, _, _, scores_out = deconvolve_spectrum(mz, intensity, charge_range=(1, 3), tolerance=50.0, tolerance_unit="ppm")
...
def test_da_tolerance_mode() -> None:
    ...
    mz = np.array([500.0, 500.501, 501.002], dtype=np.float64)
    intensity = np.array([100000.0, 70000.0, 30000.0], dtype=np.float64)
    result = deconvolve_spectrum(mz, intensity, charge_range=(1, 3), tolerance=0.01, tolerance_unit="da")
