...
# If constants are needed from peptacular, use them directly or import them
# The original code imported from ...constants.
# Check where these are used.
# C13_NEUTRON_MASS is used in score.py but maybe not here directly?
# PROTON_MASS is used in SpectrumPeak properties.

PROTON_MASS = pt.PROTON_MASS
C13_NEUTRON_MASS = pt.C13_NEUTRON_MASS
IsotopeLookup = pt.IsotopeLookup
estimate_isotopic_distribution = pt.estimate_isotopic_distribution
...
class DeconvolutedPeak[P: PeakLike]:
    ...
    def calculate_score(self, isotope_lookup: IsotopeLookup | None = None) -> None:
        ...
        # theoretical_dist is a list of IsotopicData objects (from peptacular)
        # We need to extract abundance.
        theo: list[float] = [data.abundance for data in theoretical_dist]
        score = score_isotopic_pattern(observed, theo, min_intensity=0.0, offset_range=0)
        self.score = score
...
class Graph[P: PeakLike]:
    ...
    def get_edge_data(self, source_idx: int, target_idx: int) -> GraphEdge | list[GraphEdge] | None:
    ...
    def __repr__(self) -> str:
        ...
        edges_str = {idx: (src, tgt, str(edge)) for idx, (src, tgt, edge) in self._edges.items()}
        return f"Graph(\n Nodes: {nodes_str},\n Edges: {edges_str}\n)"
